		...
		this.model.categoryModel
			...
			.then((category) => {
				if (category) {
					return res
						.status(400)
						.json({ data: [{ fields: "category", message: "category is existed" }], success: false });
				}
		...
		this.model.categoryModel
			...
			.then((categories: any) => {
				if (!categories) {
					return res
						.status(400)
						.json({ data: [{ fields: "category", message: "no categories" }], success: false });
				}
		...
		this.model.categoryModel
			...
			.then((category) => {
				if (!!category) {
					...
					return res
						.status(400)
						.json({ data: [{ fields: "category", message: "category not found!" }], success: false });
				}
			})
			.catch((err) => {
