		...
		this.model.productModel
			...
			.then((product) => {
				if (product) {
					return res
						.status(400)
						.json({ data: [{ fields: "product", message: "product is existed" }], success: false });
				}
				...
				if (productCategory.length > 0) {
					...
					Promise.all(promises)
						...
						.catch((err) => {
							return res
								.status(500)
								.json({ data: [{ fields: "product", message: err.message }], success: false });
						});
				} else {
					newProduct
						.save()
		...
		this.model.productModel
			...
			.then((product) => {
				if (!product) {
					return res
						.status(400)
						.json({ data: [{ fields: "product", message: "product not found!" }], success: false });
				}
